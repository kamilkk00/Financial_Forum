...
def save_post_detail(request, post_id):
    if request.method in ['POST', 'PUT']:
        ...
        if request.method == 'POST':
            ...
            if created: 
                return JsonResponse({"message:" "New post save created"}, status=201)
            else:
                return JsonResponse({"error": "Save_Post entry already exists "})

        elif request.method == 'PUT':
            save_post = Save_Post.objects.get(user=user, post=post)
            save_post.if_save = if_save
            save_post.save()
            return JsonResponse({"message": "Post save status updated"}, status=200)
    else: 
...
def like_function(request, post_id):
    if request.method in ['POST', 'PUT']:
        ...
        if request.method == 'POST':
            ...
            if created: 
                return JsonResponse({"message:" "New like created"}, status=201)
            else:
                return JsonResponse({"error": "Like already exists "})

        elif request.method == 'PUT':
            save_post = Like.objects.get(user=user, post=post)
            save_post.if_like = if_like
            save_post.save()
            return JsonResponse({"message": "Like status updated"}, status=200)
    else: 
        return JsonResponse({"error": "Invalid request method."}, status=405)
...
def add_opinion(request, username):
    ...
    if user == username:
        ...
        if request.method == "POST":
            ...
            message = "Opinion added succesfully"
            return render(request,"finance/add_opinion.html",{
                "message": message,
                "opinion": opinion
            })
